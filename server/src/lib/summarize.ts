...
export  const generateSummary = async (chunks: Chunk[]) => {
    const allSummaries = chunks.map((chunk) => chunk.summary).join("\n\n");
    const response = await summarizeText(allSummaries)
    ...
    //   });
      ...
      const { title, summary } = await response;
    ...
    // return summarizeText(allSummaries);
}
